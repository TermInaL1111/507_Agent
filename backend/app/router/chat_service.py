...
class ChatService:
    ...
    async def handle_rag_query(self, query: str) -> dict:
        ...
        return await rag_service.rag_summary_with_sources(query)
    ...
    async def handle_add_vector_single(self, file: UploadFile, user_id: str) -> str:
        ...
        file_metadata = {
            ...
            "doc_name": source_file.original_filename,
    ...
    async def handle_add_vector_multiple(self, files: List[UploadFile], user_id: str) -> List[str]:
        ...
        await store.get_document(files=files, user_id=user_id, file_records=file_records)
    ...
    async def handle_reorder(self, query: str, documents: List[str]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if result["success"]:
                ...
                logger.info(f"【重排序结果】查询: {query} 排序结果: {sorted_docs_log}")
